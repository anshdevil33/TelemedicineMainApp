...
public class SpareRepository {
	...
	public List<Map<String, Object>> getSpecializations() {
		return jdbcTemplate.queryForList(
				"SELECT specialization_id,shortname FROM telemedicine_master.tbl_medical_specializations");
	}

	public List<Map<String, Object>> downloadCaseSheets(int appointmentId) {
		String query = "select sheet_path from telemedicine_reports.tbl_case_sheets where appointment_id=?";
		List<String> sheet_path = jdbcTemplate.queryForList(query, String.class, new Object[] { appointmentId });
		List<Map<String, Object>> caseSheets = new ArrayList<>();
		sheet_path.stream().forEach(sheet -> {
			File file = new File(caseSheetPath + appointmentId + "\\" + sheet);
			FileInputStream fileInputStream;
			try {
				Map<String, Object> m = new HashMap<>();
				fileInputStream = new FileInputStream(file);
				m.put("filename", sheet);
	...
	public List<Map<String, Object>> downloadMedicalReports(int appointmentId) {
		...
		query.append(" select b.test_name report_name,a.report_path ");
		...
		reports.stream().forEach(report -> {
			File file = new File(medicalReportsPath + appointmentId + "\\" + report.get("report_path"));
			FileInputStream fileInputStream;
			try {
				Map<String, Object> m = new HashMap<>();
				fileInputStream = new FileInputStream(file);
				m.put("testname", report.get("report_path"));
				m.put("filename", report.get("report_path"));
				m.put("filedata", Base64.getEncoder().encodeToString(IOUtils.toByteArray(fileInputStream)));

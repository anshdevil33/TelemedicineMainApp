...
public class DoctorLoginRepository {
	...
	private Logger logger = LoggerFactory.getLogger(DoctorLoginRepository.class);
			...
			if ((long) doctor.get("COUNT") == 1) {
				if ((int) doctor.get("status") == 1001) {
					...
					int otpUp = jdbcTemplate.update(
							"INSERT INTO telemedicine_doctors.tbl_doctor_login_otp(MOBILE,OTP) values(?,?)",
					...
					m = smsService.send(otp.toString(), mobile, "DoctorLogin");
	...
	public Map<String, Object> verifyOtp(Map<String, Object> otpVerify) {
		String query = "select otp,if(TIME_TO_SEC(timediff(current_timestamp,lup_date))>300,0,1) validity from telemedicine_doctors.tbl_doctor_login_otp where mobile=? order by lup_date desc limit 1";
